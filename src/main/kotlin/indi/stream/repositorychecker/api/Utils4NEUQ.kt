...
fun registerCurriculumWithRequirements(curriculum: String, requirements: Map<String, List<FolderChecker>>) {
...
private fun List<FolderChecker>.specialize(
    studentName: String,
    weekName: String,
): SpecializedChecker =
    { file ->
        val weekFolder = file.resolve(weekName)
        (if (weekFolder.exists()) this.asSequence().map { it(weekFolder) }.flatten()
        else sequenceOf("文件夹未创建"))
...
private val String.requirements: Sequence<SpecializedChecker>
    get() = curriculumToRequirements[this.curriculum]?.asSequence()?.map { (weekName, checkers) ->
        checkers.specialize(this, weekName)
    } ?: throw IllegalArgumentException("未找到名为${this.curriculum}的学生的学习路线")

private val studentToCurriculum = mutableMapOf<String, String>()

private val curriculumToRequirements = mutableMapOf<String, Map<String, List<FolderChecker>>>()
